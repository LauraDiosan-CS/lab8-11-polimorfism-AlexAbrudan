...
string UI::color(int col = 7, string text="")
{
...
void UI::all_trains()
{
	...
	for (Train* t : all)
	{
		if (strcmp(typeid(*t).name(), "class FreightTrain") == 0)
		{
			if (t->get_av_cart_no() != 0)
				cout << ((FreightTrain*)t)->toString() << endl;
			else
			{
				cout << this->color(4, ((FreightTrain*)t)->toString()) << endl;
				this->color();
			}
		}
		else
			if (strcmp(typeid(*t).name(), "class PersonalTrain") == 0)
			{
				...
				{
					cout << this->color(4, ((PersonalTrain*)t)->toString()) << endl;
					this->color();
				}
			}
	}
...
void UI::up_da_bookings()
{
	...
	for (Train* t : all)
	{
		...
		{
			cout << this->color(4, t->toString()) << endl;
			this->color();
		}
	}
...
void UI::menu()
{
	...
	cout << "4. Delete. " << endl;
	cout << "5. Search. " << endl;
	cout << "6. Update booked number of carts for given model. " << endl;
	cout << "x. Log out. " << endl;
}
...
void UI::f()
{
	...
	try
	{
		while (true)
		{
			...
			case 'x': {system("CLS"); this->log_or_reg(); break; }
			}
		}
	}
	...
	catch (ExceptionUpdates& no_model)
	{
		cout << no_model.what() << endl << endl;
		this->f();
	}
	catch (ValidationException& t_invalid)
	{
		cout << t_invalid.what() << endl << endl;
		this->f();
	}
}